...
cc.Class({
    ...
    //得到中心点位置
    getMN(){
        var  cx = parseInt(this.node.x);
        var  cy = parseInt(this.node.y);
        cc.log("cx:",cx);
        cc.log("cy:",cy);
    ...
    positionInit(){
        ...
        this.node.x = this.sx;
        this.node.y = this.sy;
        cc.log("this.cameraNode.x:",this.node.x);
        cc.log("this.cameraNode.y:",this.node.y);
    },
    ...
    //拼接背景
    bgInit(num,block){
        var  m = parseInt(this.node.x);
        var  n = parseInt(this.node.y)-1000;
        var x = m;
        var y = n;
        var tNode = null;
        var round = 0;
